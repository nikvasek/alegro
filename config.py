...
# =============================================================================
# НАСТРОЙКИ БРАУЗЕРОВ ДЛЯ TRADEWATCH
# =============================================================================
# Настройки для управления количеством браузеров и режимом работы

# Максимальное количество одновременно работающих браузеров
# Рекомендуемые значения:
# - 1: Последовательная обработка (медленно, но стабильно)
# - 2: Базовое ускорение (оптимально для слабых компьютеров)
# - 4: Высокая производительность (рекомендуется для большинства случаев)
# - 6: Максимальная скорость (для мощных компьютеров)
# ОПТИМИЗИРОВАНО ДЛЯ 300K EAN: 2 браузера с улучшенной стабильностью
MAX_PARALLEL_BROWSERS = 1  # Изменено на 1 браузер для устранения Connection refused ошибок

# Настройки управления ресурсами для стабильности 2-браузерной работы
RESOURCE_MANAGEMENT = {
    'min_free_memory_mb': 300,        # Минимум свободной памяти для запуска браузера
    'max_chrome_processes': 100,       # Максимальное количество Chrome процессов
    'process_cleanup_interval': 60,    # Интервал очистки процессов (секунды)
    'chrome_memory_limit_mb': 512,     # Лимит памяти на браузер
    'connection_pool_size': 5,         # Размер пула HTTP соединений
    'max_retries': 3,                  # Максимальное количество повторных попыток
    'retry_delay_base': 2,             # Базовая задержка между попытками
}

# Режим работы браузеров (True - скрытый режим, False - видимые браузеры)
# True: Браузеры работают в фоновом режиме (быстрее, меньше нагрузка на систему)
# False: Браузеры видны пользователю (полезно для отладки и мониторинга)
HEADLESS_MODE = True

# Размер группы EAN кодов для обработки за один раз
# Максимум 450 кодов за запрос (ограничение TradeWatch)
# Не рекомендуется изменять без необходимости
BATCH_SIZE = 450

# Таймауты для работы браузеров (в секундах)
BROWSER_TIMEOUTS = {
    'page_load': 30,        # Время ожидания загрузки страницы
    'element_wait': 20,     # Время ожидания появления элемента
    'download_wait': 60,    # Время ожидания загрузки файла
    'login_wait': 15,       # Время ожидания входа в систему
    'export_wait': 45       # Время ожидания экспорта данных
}

# Настройки checkpoint системы для больших файлов
CHECKPOINT_SETTINGS = {
    'enabled': True,                    # Включить систему чекпоинтов
    'save_interval': 50,               # Сохранять прогресс каждые N групп
    'checkpoint_file': 'processing_checkpoint.json',  # Файл для сохранения прогресса
    'resume_on_restart': True,         # Возобновлять обработку при перезапуске
    'max_resume_attempts': 3           # Максимальное количество попыток возобновления
}

# Настройки для больших файлов (300k+ EAN)
LARGE_FILE_OPTIMIZATIONS = {
    'memory_cleanup_interval': 25,     # Очищать память каждые N групп
    'temp_file_cleanup_interval': 100, # Очищать временные файлы каждые N групп
...
RESOURCE_MANAGEMENT = {
    'browser_start_delay': 15,          # Задержка между запуском браузеров (секунды) - УВЕЛИЧЕНО для стабильности
